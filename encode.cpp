...
//globally make a tree of Null nodes
symbol* Root=NULL;

//get a new symbol
symbol* getNewSymbol(tree array[],int size)
{
	symbol* t = new symbol();
...
//insert a new node
symbol* InsertNode(tree array[],int size)
{

	if(Root==NULL)
	{
		Root = getNewSymbol(array,size);
		return Root;
	}
	// in main: root = InsertNode(root, 15);
}

//print values inside symbol whose freq is more than 0
void print_symbol(symbol S[])
{
	...
	for(int i=0;i<128;i++)
	{
		if(S[i].freq > 0)
		{
			cout << "\nI:(" << i <<")\tS:(" << S[i].symbol << ")\tF:(" << S[i].freq <<")\tE:("<<S[i].encoding<<")\n";
		}
	}
}

//print values inside tree
void print_tree(tree T[],int size)
{
	cout << "\nTree";
	for(int i=0;i<size;i++)
	{
		cout << "\nI:(" << i <<")\tIDX:("<< T[i].index<<")\tS:(" << T[i].symbol << ")\tF:(" <<T[i].freq<<")\n";
	}
}
...
int main(int argc, char* argv[])
{
	char file_name[20]="";
	char sort_method[10]="";
	//int argc = 3 (./a.out, pre-file.txt, sorting method
	if(argc==3)
	{
	...
	//printing useful elements from symbol
	print_symbol(S);
	//printing useful elements from tree
	print_tree(Alpha, Alpha_Count+1);
	print_tree(NonAlpha, NonAlpha_Count+1);

	//inputting a string to encode
	char str[20000];
